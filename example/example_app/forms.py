...
class TestForm(floppy.Form):
    ...
    checkinput = floppy.ChoiceField(choices=((0, 'Zero'), (1, 'One'),), required=False, widget=floppy.CheckboxSelectMultiple({"class": "test"}))
    ...
    textarea = floppy.CharField(widget=floppy.Textarea)
    bootstrap_textarea = floppy.MarkdownField()
    ...
    is_top = floppy.BooleanField(label=u'是否置顶', required=False)
